...
#define VECTORSTEP 5 /// Each vector is VECTORSTEP away from next vector because image has too many pixels
//每个向量都与下一个向量相距VECTORSTEP，因为图像的像素过多
#define POINTSTEP 10 /// When vector is calculated, a point is POINTSTEP away from other point(pixel)
//计算向量时，一个点与其他点（像素）相距POINTSTEP
...
public:
	...
	int SNV_point(Mat& imgDisp8, Mat& matDepth, Point ptImgLoc){
		...
		m_vec3dDirection = a.cross(b);// cross函数计算向量
		dScale = norm(m_vec3dDirection, NORM_L2);//norm计算范数  NORM_L2代表二范数 求出的是向量长度
		return 0;
	}
...
public:
	...
	int SuperSNV(vector<CSurfaceNormal>& objValid){
		...
		//输入图像 图像个数 需要统计直方图的第几通道 掩膜 输出的直方图数组 需要统计直方图通道的个数 histSize：指的是直方图分成多少个区间就是 bin的个数 统计像素值的区间

		int nHistWidth = 500; int nHistHeight = 500;
		int nBinWidth = cvRound((double)nHistWidth / nHistSize);//返回跟参数最接近的整数值，即四舍五入
