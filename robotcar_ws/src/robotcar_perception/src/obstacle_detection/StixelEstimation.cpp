#include <robotcar_perception/obstacle_detection/StixelEstimation.h>
...
CStixelEstimation::CStixelEstimation(StereoCamParam_t& objStereroParam)  
{
	...
	m_dGroundVdispSlope = 0;//视差中的地线斜率
	m_dGroundVdispOrig = 0;//　v视差中的地线原点

	m_nVdispGroundThreshold = 50;//视差图中的地平面阈值
	m_nStixelGroundMargin = 15;//10;//接地平面的边际补偿值

	m_imgGround = Mat(m_objStereoParam.objCamParam.m_sizeSrc, CV_8U);
	m_imgGround = Scalar(0);//scalar是将图像设置成单一灰度和颜色,这里将地面设置为单一颜色
}
...
STIXEL_ERROR CStixelEstimation::EstimateStixels(Mat& matDisp16)
{
	...
	RmSky(m_imgDisp8);
	morphologyEx(m_imgDisp8, m_imgDisp8, MORPH_OPEN, matKernel, Point(-1, -1), 1);//morphologyEx这个函数可以方便的对图像进行一系列的膨胀腐蚀组合
...
{
	int maxDisp = 255;
	m_imgVdisp = Mat(imgDisp8.rows, 255, CV_8U, Scalar(0));//创建一个和原始图像imgDisp8高一致的8位无符号单通道的灰度图片容器,并且初始化图片为黑色0
...
STIXEL_ERROR CStixelEstimation::RmVDisparityNoise(Mat& imgVdisp)
{
	...
	threshold(imgVdisp, imgVdisp, nThresh, 255, 3); //图像二值化 为3 代表dst(x,y)={ thresh if(src(x,y)>thresh) ,否则为src(x,y)} 
	return OK;
}
...
STIXEL_ERROR CStixelEstimation::FitLineRansac(vector<Point2f>& vecLinePoint, Vec4f& vec4fLine)//RANSAC算法在三维空间中进行路面估计
{
...
STIXEL_ERROR CStixelEstimation::StixelDistanceEstimation(Mat& imgDisp8, vector<stixel_t>& vecStixels, bool flgUseMultiLayer)
{
	STIXEL_ERROR Err;
	vecStixels.clear();
	for (int u = 0; u < imgDisp8.cols; u += m_nStixelWidth){
...
STIXEL_ERROR CStixelEstimation::StixelDisparityEstimation_col_ML(Mat& imgDisp8, int col, vector<stixel_t>& vecStixels)
{
	...
	for (int v = imgDisp8.rows - 1; v >= nSkybound; v--){
		...
		if (chDisp > 0 && objStixelTemp.nGround == -1 && v > m_objStereoParam.objCamParam.m_nVanishingY - 10){
			...
			if (m_objStereoParam.objCamParam.m_nVanishingY > v)
			{
...
STIXEL_ERROR CStixelEstimation::StixelDisparityToDistance(vector<stixel_t>& vecStixels)
{
	for (unsigned int u = 0; u < vecStixels.size(); u++){
		...
		vecStixels[u].dY = - vecStixels[u].dZ*(double)(vecStixels[u].nHeight - m_objStereoParam.objCamParam.m_sizeSrc.height / 2) / m_objStereoParam.objCamParam.m_dFocalLength;
		//cout<<"vecStixels[u].dY:"<<vecStixels[u].dY<<endl;
	}
	return OK;
}
